...
const TrainingPlan_model_1 = __importDefault(require("../models/TrainingPlan.model"));
const Workout_model_1 = __importDefault(require("../models/Workout.model"));
const User_model_1 = __importDefault(require("../models/User.model"));
class TrainingPlanController {
    async getCurrentPlan(req, res) {
        try {
            const plan = await TrainingPlan_model_1.default.findOne({
                userId: req.userId,
                status: 'active'
            });
            if (!plan) {
    ...
    async generatePlan(req, res) {
        try {
            ...
            const existingPlan = await TrainingPlan_model_1.default.findOne({
                userId: req.userId,
                status: 'active'
            });
            if (existingPlan) {
            ...
            const trainingPlan = new TrainingPlan_model_1.default({
                userId: req.userId,
    ...
    async updateSession(req, res) {
        try {
            ...
            for (const week of plan.weeklyPlan) {
                const session = week.sessions.find((s) => s._id.toString() === sessionId);
                if (session) {
                    session.completed = completed;
                    if (actualWorkoutId)
                        session.actualWorkoutId = actualWorkoutId;
                    if (notes)
                        session.notes = notes;
                    sessionUpdated = true;
                    break;
    ...
    calculateBaselineMetrics(workouts) {
    ...
    generateWeeklyPlan(user, baselineMetrics) {
