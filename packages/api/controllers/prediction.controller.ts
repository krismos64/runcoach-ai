...
export class PredictionController {
  async getNextWorkoutPrediction(req: AuthRequest, res: Response) {
    try {
      ...
      const prediction = await mlPredictionService.predictNextWorkout({
        recentWorkouts,
        userProfile: {
          age,
  ...
  async getPerformancePrediction(req: AuthRequest, res: Response) {
    try {
  ...
  async getRaceTimePrediction(req: AuthRequest, res: Response) {
    try {
      const { distance = 21097.5 } = req.query;

      const recentRuns = await Workout.find({
        userId: req.userId,
  ...
  private calculatePerformanceMetrics(workouts: any[]) {
    ...
    if (newAvgPace < oldAvgPace * 0.97) progressTrend = 'improving';
    else if (newAvgPace > oldAvgPace * 1.03) progressTrend = 'declining';
  ...
  private calculateVDOT(workout: any): number {
  ...
  private predictRaceTime(vdot: number, distance: number): number {
